...
class SmartVisionProcessor:
    def __init__(self):
        ...
        # MediaPipe disabled for lightweight deployment
    ...
    def gesture_control_mode(self, frame):
        """Gesture control disabled for lightweight deployment"""
        cv2.putText(frame, "Gesture Control: Disabled for Cloud", (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        cv2.putText(frame, "Use keyboard shortcuts instead", (10, 60), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                # Enhanced gesture recognition
                total_fingers = sum(fingers_up)
                
                if total_fingers == 0:
                    gesture = "Fist - Stop Recording"
                elif total_fingers == 1 and fingers_up[1] == 1:
                    gesture = "Point - Select Object"
                elif total_fingers == 2 and fingers_up[1] == 1 and fingers_up[2] == 1:
                    gesture = "Peace - Take Photo"
                elif total_fingers == 5:
                    gesture = "Open Hand - Start Recording"
                elif total_fingers == 3:
                    gesture = "Three - Change Mode"
                else:
